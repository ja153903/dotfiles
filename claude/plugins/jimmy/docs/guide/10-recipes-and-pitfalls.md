...
- **Treating `inherit` as a fixed model choice.** `inherit` means "omit the model field so the subagent runs on whatever model this session is currently on." It isn't pinned: switch your session's model mid-conversation and every `inherit` role follows, panels included, since a panel's list length still sets its size. [Setup](./01-setup.md) covers the roles.
