...
**PRs.** Run `/no-comments` over the diff before commit and again before review, and ask in plain words for the code cleanup it doesn't reach: unsupported guards, dead compatibility paths, unrelated edits. `/unslop` is prose-only and does not clean the diff. Write every PR title, PR description, and commit body with `/technical-writing`, then apply `/unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.

**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `jimmy` or `mode`, as the scope. Keep the subject short and imperative. Apply the same `/technical-writing` and `/unslop` pass as the body. Name a real symbol when one carries the change. For example, `fix(jimmy): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as the repo's generated verification skill, `/run`, or the targeted tests. State the outcome of each check, not only the command name.
...
A subagent that opens a PR runs `interrogate`, `/no-comments` on the diff, and `/unslop` on the PR prose. It returns the URL and does not babysit. Return to the parent.
