...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the Agent tool. State reads and writes go through `${CLAUDE_PLUGIN_ROOT}/skills/poteto-mode/scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
...
- **Worker / verifier.** Always `isolation: "worktree"` unless the task needs *this* machine: a browser, simulator, or physical device driven here; local IDE state; auth that exists only here. Runtime verification is not itself an exception. The project's generated verification skill (from `/create-verification-skill`) and the bundled `/run` both work inside a worktree, and the generator's interview asks whether two instances can run side by side precisely so they can. Transcripts are not an exception either: they live under `~/.claude/projects/<slug-of-cwd>/`, readable by absolute path from any checkout, but Claude Code keys that directory on cwd, so a worktree agent writes its own session somewhere else and a brief that wants this session's trail must name the parent's directory. A worktree agent sees a separate checkout, so its brief inlines what it needs or points at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
Create `orchestrate/<project-slug>/` in the current agent's store under `~/.claude/projects/<slug>/`, where the slug is the session's working directory with every `/` and `.` replaced by `-`. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Use `bun "${CLAUDE_PLUGIN_ROOT}/skills/poteto-mode/scripts/orch/orch.ts"` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Standing orders are pasted verbatim into every spawn and every resume, never referenced by path; directives decay across resumes, and each dropped one costs a human turn.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. A restack at this scale takes the laptop down, so keep it serialized and expect it to be slow; `isolation: "remote"` is the escape hatch where the user has access to it, not this plugin's default.
...
A unit is not done until its output is externalized the moment it lands, never batched to the end of the run: a worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in one worktree when that worktree's session dies was never done.

#### Liveness and failure

- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the agent's status reported by the harness. Transcript mtime is not liveness.
...
- After a session restart, every agent is dead; nothing survives to reattach to. Re-read the standing orders and `units.tsv`, recompute the frontier from PR and branch state on the forge (that is how you identify which unit is which, not a live agent id), respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
