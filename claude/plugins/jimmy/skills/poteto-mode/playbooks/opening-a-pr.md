...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as the repo's generated verification skill, `/run` and `/verify`, or the targeted tests. State the outcome of each check, not only the command name.
