---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via writing-skills + unslop, optionally pulling fresh evidence from recent transcripts."
...
This skill orchestrates three others: an inline mining pass (see step 1), the **writing-skills** skill (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Locate the active project's transcripts before fanning out. The system prompt names the project path; its transcripts are the `.jsonl` files under `~/.claude/projects/<slug>/`. Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses project boundaries and reads private chats from unrelated projects.
...
Use the **writing-skills** skill to author the skill. Placement:
...
- Frontmatter formatting: follow **writing-skills**'s YAML rules. Keep `description` as one YAML scalar; quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
...
Apply the **unslop** skill and `superpowers:writing-skills`' guidelines to every line. Both apply to any agent-read prose, not just skills.
...
A `-mode` skill is subjective output. A **writing-skills**-style test/iterate benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill (not working conventions): **writing-skills** alone, no mining required.
...
- The **writing-skills** skill: skill authoring process and writing guidelines.
