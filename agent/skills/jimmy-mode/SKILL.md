...
hooks:
  UserPromptSubmit:
    - hooks:
        - type: command
          command: "$HOME/.claude/skills/jimmy-mode/scripts/mode-reminder.sh"
---
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the **superpowers:writing-skills** skill (the skill for authoring SKILL.md files).
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → clean the diff. The **no-comments** skill (`/no-comments`) takes the narrating comments; ask in plain words for what it doesn't reach, such as unsupported guards, dead compatibility paths, and unrelated edits. **unslop** is prose-only, so it does not cover the diff.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → this repo's generated verification skill (from `/create-verification-skill`) when one exists, or Claude Code's bundled `/run` otherwise. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
- Any PR-status request → the **Babysit** playbook (`playbooks/babysit.md`), and not any other built-in babysit skill, whose description matches the same words. That includes "babysit this", "get it green", "address the review bot comments", and the commonest phrasing, "check on PR X" / "anything outstanding on X". Never triggered by merely opening a PR. Declare its mode before polling; the playbook's step 1 owns the request-to-mode mapping. Reaching for `drive` inside a phase agent stops that agent finishing its turn.
- Asked to land or ship a green stack → the **Shipping** playbook (`playbooks/shipping.md`). Green is not safe. Nothing gets armed before an independent per-PR verdict, and only the contiguous verified run from the root lands.
- An automated code-review bot or the agentic security review commented → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/review-bot-triage.md`.
